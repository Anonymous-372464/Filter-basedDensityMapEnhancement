...
function getTickValues(
  minVal: number,
  maxVal: number,
  transfromFunc: (x: number) => number
) {
  let interval = maxVal - minVal,
    base = 1;
  while (interval / base > 10) {
    base *= 10;
  }
  let tickValues = [],
    start = Math.ceil(minVal / base);
  for (let i = start, end = Math.ceil(maxVal / base); i < end; ++i) {
    tickValues.push(i * base);
  }
  if (tickValues[0] > minVal) {
  ...
  if (
    tickValues[tickValues.length - 1] < maxVal &&
    (transformed[3] - transformed[2]) / (transformed[3] - transformed[0]) > 0.02
  ) {
    tickValues.push(maxVal);
  }
  if (
    base > 1 &&
    (transformed[1] - transformed[0]) / (transformed[3] - transformed[0]) > 0.5
  ) {
    // not enough ticks for smaller values
    base /= 10;
...
function translateBottomLeftPosition(
  x: number,
  y: number,
  viewer: typeof DensityMapViewerVue
) {
  let relativeX = Math.round((x / viewer.params.width) * viewer.level0_Width),
    relativeY = Math.round(
      (1 - y / viewer.params.height) * viewer.level0_Height
    );
  ...
  let halfW = viewer.level0_Width / 2,
    halfH = viewer.level0_Height / 2,
    sizeRatio = Math.pow(2, viewer.zoomLevel),
    zoomedLeftX = value_limit(
      zoomedCenterX - halfW,
      0,
      viewer.level0_Width * (sizeRatio - 1)
    ),
    zoomedBottomY = value_limit(
      zoomedCenterY - halfH,
      0,
      viewer.level0_Height * (sizeRatio - 1)
    );
